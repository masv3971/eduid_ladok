...
func New(ctx context.Context, config Config, wg *sync.WaitGroup, feedName string, ladok *ladok.Service, eduid *eduidiam.Service, logger *logger.Logger) (*Service, error) {
	s := &Service{
		logger:   logger,
		config:   config,
		ladok:    ladok,
