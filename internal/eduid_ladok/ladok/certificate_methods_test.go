...
func mockNewCertificateService(t *testing.T) *CertificateService {
	service := &Service{
		config: Config{
			LadokAtomURL:             "",
			LadokRestURL:             "",
			LadokAtomEndpoints:       []string{},
			LadokCertificatePath:     t.TempDir(),
...
func mockNewCertificateBundle(t *testing.T, notBefore, notAfter int, path, fileType, password string) {
	...
	certName := strings.Join([]string{"testSchoolName", fileType}, ".")
...
func TestUnwrapBundle(t *testing.T) {
	...
	for _, tt := range tts {
		...
		mockNewCertificateBundle(t, tt.have.notBefore, tt.have.notAfter, cs.Service.config.LadokCertificatePath, tt.have.fileType, cs.Service.config.LadokCertificatePassword)

		if err := cs.UnwrapBundle(); err != nil {
			assert.EqualError(t, err, tt.want.Error())
		}

		assert.NotNil(t, cs.CRT, "client cert should not be nil")
		assert.NotNil(t, cs.PrivateKey, "privatekey should not be nil")
		assert.NotEmpty(t, cs.CRTPEM)
		assert.NotEmpty(t, cs.PrivateKeyPEM)
